...
function check_versions_consistent () {
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./charts/mosquitto-go-auth/values.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./charts/mosquitto-go-auth/Chart.yaml)

  if [ "$HELM_CHART_APP_VERSION" != "$HELM_VALUES_TAG_VERSION" ]; then
...
CURRENT_VERSION=$(yq eval '.version' ./charts/mosquitto-go-auth/Chart.yaml)
